...
@fdb.transactional
def add(tr, bsize, dsize):
    for i in range(bsize):
        data = generate_data(dsize).encode('utf-8')
        now = str(datetime.utcnow().timestamp())
        uid = str(uuid.uuid4().hex)
        key = pack((now, uid))
        tr.set(key, data)

def add_batch_process(lim, bsize, dsize):
    db = fdb.open()
    i = 0
...
p = Pool(num_proc)
multiple_results = [p.apply_async(add_batch_process, (num_records, batch_size, data_size,)) for i in range(num_proc)]
